...
def get_youtube_credentials_from_file(
...
def get_youtube_credentials_from_oauth(
    oauth_credentials_path: pathlib.Path,
) -> Credentials:
...
def get_youtube_credentials(oauth_credentials_path: pathlib.Path) -> Credentials:
    ...
    youtube_credentials_path = (
        pathlib.Path(__file__).parent.parent / ".config" / "youtube_credentials.dat"
    )
    youtube_credentials = None
    try:
        LOGGER.debug("loading youtube credentials from file")
        youtube_credentials = get_youtube_credentials_from_file(
            youtube_credentials_path
        )
...
def update_broadcast_description(
    credentials: Credentials, broadcast_data: BroadcastData, new_description: str
) -> None:
