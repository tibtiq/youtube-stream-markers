    ...
    class Test_asserts:
        def test_path_doesnt_exist(self, tmp_path):
            ...
            expected_assert_message = (
                "The provided path for api credentials doesn't exist"
            )
        ...
        def test_path_file_type_incorrect(self, tmp_path):
            ...
            with pytest.raises(AssertionError) as info:
                get_youtube_credentials_from_file(temp_file)
            assert expected_assert_message in str(info.value)
...
class Test_get_youtube_credentials_from_oauth:
    class Test_asserts:
        ...
        def test_path_file_type_incorrect(self, tmp_path):
            temp_file = tmp_path / "file.txt"
            temp_file.write_text("Hello World", encoding="utf-8")
            expected_assert_message = "The provided path for api credentials appears to be incorrect file type."

            with pytest.raises(AssertionError) as info:
                get_youtube_credentials_from_oauth(temp_file)
            assert expected_assert_message in str(info.value)
